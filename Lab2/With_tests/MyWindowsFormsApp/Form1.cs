...
namespace MyWindowsFormsApp
{
    public partial class Form1 : Form
    {
        ...
        public List<T> GenerateSampleEmployees<T>(string employeeType, int count) where T : Employee
        {
            ...
            {
                T employee = EmployeeFactory.CreateEmployee(employeeType, i, $"John{i}", $"Doe{i}", "Software Development", 1000 + (i * 200), 160 + i) as T;
                if (employee != null)
                {
        ...
        public void UpdateChart(double partTimeMean, double hiredMean, double fullTimeMean)
        {
            ...
            chart.AxisY.Add(new Axis { Title = "Mean Monthly Salary", LabelFormatter = value => value.ToString("C", System.Globalization.CultureInfo.CurrentCulture) });
        }
